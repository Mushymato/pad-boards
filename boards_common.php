			...
			foreach($board['styles'] as $name => $style){
				foreach($style as $color => $count){
					if(in_array($color, $rgbld_orb_list) || $color == 'H'){
						...
						$style_str = $style_str . '<div class="style-box">' . orb_style_icon($color, $name) . '<span>&times;' . $count . '</span></div>';
					}
				}
			}
		}
...
function get_attribute_filters($boards){
	...
	foreach($colors as $orb){
		$out = $out . '<div class="grid filters" data-orb-base="' . $orb . '"><div class="grid atts">' . orb_radios($orb) . '</div><div class="orb-count">&ge;<input class="orb-num" type="text" maxlength="2" value="0"><input class="slider" type="range" min="0" max="' . $max . '" value="0"><div class="selected-styles"></div>' . $max . '</div><div class="float style-buttons">';
		sort($styles[$orb]);
